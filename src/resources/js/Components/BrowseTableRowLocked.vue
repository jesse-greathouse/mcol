<template>
  ...
  <browse-table-cell-completed
    v-if="isStatus('completed')"
    :packet="packet"
    :settings="settings"
  ...
  />
  <browse-table-cell-downloading
    v-else-if="isStatus('incomplete')"
    :packet="packet"
    :settings="settings"
    ...
    @call:saveDownloadDestination="saveDownloadDestination"
  />
  <browse-table-cell-queued
    v-else-if="isStatus('queued')"
    :packet="packet"
    :settings="settings"
  ...
  <td class="border-t bg-gray-100">
    <span class="flex items-center px-6 py-4" tabindex="-1">
      <span class="text-xs font-medium me-2 px-2.5 py-0.5 rounded border" :class="nickClass">
        {{ packet.nick }}
      </span>
    </span>
  </td>
