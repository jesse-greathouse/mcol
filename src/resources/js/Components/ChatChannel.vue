<template>
  ...
  <div class="relative flex flex-row content-end gap-4 grow">
    ...
    <div
      ref="chatPane"
      class="flex flex-col content-end overflow-y-auto scroll-smooth w-full max-w-full mr-3"
    ...
    <!-- Start User List-->
    <div
      class="flex flex-col overflow-y-auto overflow-x-hidden w-96 px-3"
      :style="{ maxHeight: chatPaneHeight }"
    >
...
export default {
  ...
  data() {
    ...
    const { state: offsetState, saveState: saveOffsetState } = usePageStateSync(offsetKey, {
      messageOffset: 0,
      eventOffset: 0,
    });
    ...
    saveLinesToStorage(max = 30) {
      ...
      try {
        localStorage.setItem(
          `chat:buffer:${this.network}:${this.cleanChannelName}`,
