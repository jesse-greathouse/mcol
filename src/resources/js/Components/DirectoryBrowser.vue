<template>
  <div class="relative shadow-xl bg-white rounded-lg shadow dark:bg-gray-700">
    ...
    >
      <div class="w-full grid gap-3 grid-cols-12 grid-rows-3">
        <div class="col-span-3">
          <multiselect
            tabindex="-1"
            class="hover:text-gray-700 focus:text-indigo-500 text-sm"
        ...
        <div class="col-span-12">
          <input
            type="text"
            class="min-w-60 relative px-6 py-3 w-full rounded focus:shadow-outline"
            :disabled="true"
            v-model="displayUri"
            ref="displayUri"
          />
        ...
        <div class="col-span-12">
          <div class="inline-flex">
            <button
              ref="up"
              type="button"
              class="text-white disabled:opacity-75 disabled:bg-gray-100 focus:outline-none font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center me-2 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:z-10 focus:ring-0 dark:bg-gray-800 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-700"
    ...
    <!-- body -->
    <div
      class="relative overflow-x-auto overflow-y-auto shadow-md sm:rounded-lg max-h-132 min-h-48"
    >
      <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        >
          <tr>
            <th scope="col" class="cursor-pointer px-6 py-3">File Name</th>
            <th scope="col" class="cursor-pointer px-6 py-3">Size</th>
            <th scope="col" class="cursor-pointer py-3">Modified</th>
          </tr>
        </thead>
        <tbody>
          <directory-browser-row
            v-for="file in directoryTable"
            :key="file.uri"
            :settings="settings"
            :file="file"
    ...
    >
      ...
      <button
        type="button"
        class="text-white bg-green-500 hover:bg-green-600 focus:ring-4 focus:outline-none focus:ring-green-200 font-medium rounded-lg text-sm px-5 py-2.5 ms-3 text-center dark:bg-green-400 dark:hover:bg-green-500 dark:focus:ring-green-600"
  ...
  <div
    data-popover
    class="invisible absolute inline-block z-50 w-auto rounded-lg border border-gray-200 bg-white text-sm text-gray-500 opacity-0 shadow-sm transition-opacity duration-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400"
    ref="createDirPop"
    :id="createDirPopId"
    role="tooltip"
  >
    <div class="p-3">
      ...
      <div class="flex flex-row p-3 justify-end">
        <div class="flex">
          <button
            type="button"
            class="inline-flex items-center me-2 text-center font-medium rounded-lg text-sm focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 inline-flex items-center focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:z-10 focus:ring-0 dark:bg-gray-800 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-700"
          ...
          <button
            type="button"
            class="focus:ring-4 focus:outline-none inline-flex items-center me-2 text-center text-white font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 bg-green-400 hover:bg-green-500 focus:ring-green-200 font-medium rounded-lg text-sm p-2.5 dark:bg-green-400 dark:hover:bg-green-400 dark:focus:ring-green-500"
            @click="createDir()"
          >
            Create
          </button>
...
export default {
  ...
  mounted() {
    this.createDirPop = new Popover(
      this.$refs.createDirPop,
      this.$refs.new,
      {
        placement: 'bottom',
        triggerType: 'none',
    ...
    async refreshDir() {
      ...
      if (error === null) {
        ...
        if (error.code === 'ERR_BAD_REQUEST') {
          // The directory hasn't been created yet — this is normal in some cases.
          console.log(error.message);
        } else {
          // Unexpected error — log it as an actual problem.
          console.error(error);
        }
