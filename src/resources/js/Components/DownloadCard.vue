<template>
    <div :id="download.packet.file_name"
        ...
        <div class="absolute right-2 top-1/2 transform -translate-y-1/2 flex flex-col items-center space-y-2">
            <save-download-button context="download-card" :download="download" :settings="settings"
                @call:saveDownloadDestination="saveDownloadDestination" />

            <!-- Queued State -->
            <button v-if="isQueued" ref="remove" type="button"
                ...
                :disabled="cancelDisabled" @click="remove()">
                <cancel-icon />
                <span class="sr-only">Remove Queue</span>
            </button>

            <!-- Incomplete State -->
            <button v-if="isDownloading" ref="cancel" type="button"
                class="text-white disabled:opacity-75 disabled:bg-gray-700 bg-red-400 hover:bg-red-500 focus:ring-4 focus:outline-none focus:ring-red-200 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center me-2 dark:bg-red-400 dark:hover:bg-red-400 dark:focus:ring-red-500"
                :disabled="cancelDisabled" @click="cancel()">
                <cancel-icon />
                <span class="sr-only">Cancel Download</span>
            </button>

            <!-- Completed State -->
            <cancel-completed-download-button v-if="isCompleted" :disabled="cancelDisabled" :download="download"
