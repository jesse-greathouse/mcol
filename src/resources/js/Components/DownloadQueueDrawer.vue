<template>
    <!-- drawer component -->
    <div ref="queueDrawer"
        class="shadow-[0_30px_40px_15px_rgba(0,0,0,0.3)] fixed z-50 w-full overflow-y-auto bg-white border-t border-gray-200 rounded-t-lg dark:border-gray-700 dark:bg-gray-800 transition-transform translate-y-full bottom-0 left-0 right-0 top-1/2 bottom-[80px]"
        id="drawer-swipe" tabindex="-1">
        <div ref="drawerHeader" class="p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700"
            ...
            <h5 id="drawer-swipe-label"
                class="font-semibold text-slate-600 inline-flex items-center text-base text-gray-500 dark:text-gray-400 font-medium text-nowrap">
        ...
        <div class="grid grid-cols-1 gap-4 p-4 lg:grid-cols-1">
            <div v-if="hasCompleted" class="p-4 rounded-lg bg-gray-50 dark:bg-gray-700">
                ...
                <div v-for="download in queue.completed" :key="`download-completed-${download.id}`" class="width-full">
            ...
            <div v-if="hasDownloading" class="p-4 rounded-lg bg-gray-50 dark:bg-gray-700">
                ...
                <div v-for="download in queue.incomplete" :key="`download-incomplete${download.id}`" class="width-full">
            ...
            <div v-if="hasQueued" class="p-4 rounded-lg bg-gray-50 dark:bg-gray-700">
                ...
                <div v-for="download in queue.queued" :key="`download-queued-${download.id}`" class="width-full">
