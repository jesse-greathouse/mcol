<template>
  <div class="flex items-center m-3">
    <label class="inline-flex items-center cursor-pointer">
      <input v-bind:checked="exclude" @input="$emit('update:excludeDynamicRange', $event.target.checked)" type="checkbox" value="" class="sr-only peer">
  ...
  <ul class="flex items-center text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg sm:flex dark:bg-gray-700 dark:border-gray-600 dark:text-white">
      <li v-for="dynamic_range in dynamic_ranges" ref="items" class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
          <div class="flex flex-nowrap items-center ps-2">
              <input :ref="`${dynamic_range}_box`" v-bind:checked="dynamicRangeSelected(dynamic_range)" v-bind:id="dynamic_range" @input="$emit('update:dynamicRanges', dynamic_range, $event.target.checked)" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" :class="accent, color, ring">
              <label for="{{ dynamic_range }}" class="w-max py-3 ms-1 mr-2 text-sm font-medium text-gray-900 dark:text-gray-300 capitalize">{{ label(dynamic_range) }}</label>
          </div>
      </li>
  </ul>
</template>
