<template>
  <div class="flex items-center m-3">
    <label class="inline-flex items-center cursor-pointer">
      <input
        v-bind:checked="exclude"
        @input="$emit('update:excludeDynamicRange', $event.target.checked)"
        type="checkbox"
        value=""
        class="sr-only peer"
      />
  ...
  >
    <li
      v-for="dynamic_range in dynamic_ranges"
      :key="`dynamic-range-option-${dynamic_range}`"
      ref="items"
      class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600"
    >
      <div class="flex flex-nowrap items-center ps-2">
        <input
          :ref="`${dynamic_range}_box`"
          v-bind:checked="dynamicRangeSelected(dynamic_range)"
          v-bind:id="dynamic_range"
          @input="$emit('update:dynamicRanges', dynamic_range, $event.target.checked)"
          type="checkbox"
