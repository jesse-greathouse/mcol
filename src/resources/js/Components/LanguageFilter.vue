<template>
  <div class="flex w-auto items-center m-3">
    <label class="inline-flex items-center cursor-pointer">
      <input v-bind:checked="exclude" @input="$emit('update:excludeLanguage', $event.target.checked)" type="checkbox" value="" class="sr-only peer">
  ...
  <ul class="items-center w-auto text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg sm:flex dark:bg-gray-700 dark:border-gray-600 dark:text-white">
      <li v-for="language in languages" ref="items" class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
          <div class="flex items-center ps-2">
              <input :ref="`${language}_box`" v-bind:checked="languageSelected(language)" v-bind:id="language" @input="$emit('update:languages', language, $event.target.checked)" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" :class="accent, color, ring">
