  ...
  <div class="flex w-auto items-center m-3">
    <label class="inline-flex items-center cursor-pointer">
      <input
        ...
        @input="$emit('update:excludeLanguage', $event.target.checked)"
        type="checkbox"
        value=""
        class="sr-only peer"
      />
  ...
  >
    ...
    >
      <div class="flex items-center ps-2">
        <input
          :ref="`${language}_box`"
          v-bind:checked="languageSelected(language)"
          v-bind:id="language"
          @input="$emit('update:languages', language, $event.target.checked)"
          type="checkbox"
