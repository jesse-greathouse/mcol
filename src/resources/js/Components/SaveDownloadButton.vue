<template>
    <button ref="saveFile" type="button"
        class="text-white disabled:opacity-75 disabled:bg-gray-700 bg-blue-400 hover:bg-green-500 focus:ring-4 focus:outline-none focus:ring-blue-200 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center me-2 dark:bg-blue-400 dark:hover:bg-green-400 dark:focus:ring-blue-500"
        :class="saveClass" :disabled="disableSaveFile" @click="saveDownloadDestination()">
        <svg class="stroke-white fill-none stroke-2 w-6 h-6" aria-hidden="true" viewBox="0 0 24 24" version="1.1"
    ...
    <div data-popover
        class="invisible absolute inline-block w-auto rounded-lg border border-gray-200 bg-white text-sm text-gray-500 opacity-0 shadow-sm transition-opacity duration-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400"
        ref="destinationPop" :id="destinationPopId" role="tooltip">
        <div class="p-3">
            ...
            <div class="flex flex-row p-3">
                <div class="basis-1/3 px-3">
                    <multiselect tabindex="-1" class="min-w-48 p-1 hover:text-gray-700 focus:text-indigo-500 text-sm"
                        v-model="destinationForm.root" :options="destinationRoots" :canClear="false"
                        ref="destinationRoot" />
                </div>
                <div class="basis-2/3">
                    <input type="text" class="min-w-60 relative px-6 py-3 w-full rounded focus:shadow-outline"
                        v-model="destinationForm.uri" ref="destinationUri" />
                </div>
            </div>
            <div class="flex flex-row p-3 justify-end">
                <div class="flex">
                    <button type="button"
                        class="focus:ring-4 focus:outline-none inline-flex items-center me-2 text-center text-white font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 bg-blue-400 hover:bg-blue-500 focus:ring-blue-200 font-medium rounded-lg text-sm p-2.5 dark:bg-blue-400 dark:hover:bg-blue-400 dark:focus:ring-blue-500"
                        @click="toggleBrowser()">
                        Browse Files
                    </button>
                    <button type="button"
    ...
    <teleport to="body">
        <div tabindex="-1" aria-hidden="true"
            class="fixed hidden overflow-y-hidden overflow-x-hidden z-[1000] justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] h-5/6"
            ref="directoryBrowserModal" :id="modalId">
            <div class="relative p-4 w-full max-w-5xl h-full">
                <!-- Modal content -->
                <directory-browser ref="directoryBrowser" :settings="settings" :mediaType="download.media_type"
                    :root="destinationForm.root" :uri="destinationForm.uri"
