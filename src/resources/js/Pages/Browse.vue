            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-2.5" :class="contentClass">
                ...
                <div class="flex items-start justify-start mb-4">
                    <div class="relative flex w-1/5 min-w-72 m-0 mr-4">
                        <search-filter :model="form.search_string" class="flex w-full"
                            @update:searchString="updateSearchString" @reset="reset" />
                        ...
                        <button v-if="searchStringActive" type="button" @click="resetSearchString"
                            class="text-white absolute end-2 bottom-2 bg-slate-200 hover:bg-slate-300 focus:ring-1 focus:outline-none focus:ring-slate-50 font-medium rounded-lg text-sm px-4 py-2 dark:bg-slate-400 dark:hover:bg-slate-500 dark:focus:ring-slate-300">Clear</button>
                    </div>
                    <span class="mx-2 w-48 min-w-48">
                        <Multiselect placeholder="Media" mode='multiple' @change="updateMediaType"
                            class="p-1 hover:text-gray-700 focus:text-indigo-500 text-sm" :multipleLabel="mediaLabel"
                            v-model="form.in_media_type" :options="media_types" ref="media" />
                    </span>
                    <span class="mx-2 w-48  min-w-48" v-if="filteringVideoFormat">
                        <Multiselect placeholder="Resolution" mode='multiple' @change="updateResolution"
                            class="p-1 hover:text-gray-700 focus:text-indigo-500 text-sm"
                            :multipleLabel="resolutionLabel" v-model="form.in_resolution" :options="resolutions"
                            ref="resolution" />
                    </span>

                    <span class="flex items-start justify-start mx-2" v-if="filteringVideoFormat">
                        <dynamic-range-filter ref="dynamic-ranges" @update:dynamicRanges="updateDynamicRanges"
                            @update:excludeDynamicRange="updateExcludeDynamicRanges" :exclude="exclude_dynamic_ranges"
                            :in_dynamic_range="form.in_dynamic_range" :out_dynamic_range="form.out_dynamic_range"
                ...
                <div class="flex items-start justify-start mb-4">
                    <span class="mx-2 w-48 min-w-48">
                        <Multiselect placeholder="Networks" mode='multiple' @change="updateNetwork"
                            class="p-1 hover:text-gray-700 focus:text-indigo-500 text-sm" :multipleLabel="networkLabel"
                            v-model="form.in_networks" :options="networks" ref="networks" />
                    </span>

                    <language-filter ref="languages" class="w-full max-w-md" @update:languages="updateLanguages"
                        @update:excludeLanguage="updateExcludeLanguages" :exclude="exclude_languages"
                        :in_language="form.in_language" :out_language="form.out_language" :languages="languages" />

                    <div class="flex items-center mx-6 min-w-52">
                        <vue-tailwind-datepicker ref="fromDate" v-model="form.start_date" as-single
                            placeholder="Search From" :formatter="{
                                date: 'YYYY-MM-DD',
                                month: 'MMMM',
                            }" />
                    </div>
                    <div v-show="showQueue" class="text-center">
                        <button ref="downloadsButton" type="button"
                ...
                <div class="bg-white rounded-md shadow overflow-x-auto">
                    <table class="w-full whitespace-nowrap">
                        <browse-table-head :currentOrder="form.order" :currentDirection="form.direction"
                            @call:toggleSort="toggleSort" />
                        <browse-table-body :packets="packets" :locks="locks" :completed="completed"
                            :incomplete="incomplete" :queued="queued" :settings="settings"
                            @call:requestDownload="requestDownload" @call:removeCompleted="removeCompleted"
                            @call:requestRemove="requestRemove" @call:requestCancel="requestCancel"
                ...
                <div v-show="showQueue">
                    <download-queue-drawer ref="queue" :queue="downloadQueue" :settings="settings"
                        @call:removeCompleted="removeCompleted" @call:requestRemove="requestRemove"
                        @call:requestCancel="requestCancel" @call:saveDownloadDestination="saveDownloadDestination" />
                </div>
...
export default {
    ...
    data() {
        ...
        const exclude_dynamic_ranges = (form.in_dynamic_range?.length === 0 && form.out_dynamic_range?.length > 0)
